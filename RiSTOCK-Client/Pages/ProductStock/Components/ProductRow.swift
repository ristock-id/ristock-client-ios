...
struct ProductRow: View {
    let index: Int
    @Binding var product: ProductSummaryUI // TODO: Ini masih pake mock data, harus diganti
...
//    let priority: String // "High" = 🔥
    ...
    private var isHighPriority: Bool {
        return product.checkRecommendation.filterString.lowercased() == "high"
    }
    
    // Logic warna berdasarkan minStock
    private var statusColor: Color {
        guard let stock = product.stockAmount else { return .blue }
        
        guard let rop = product.rop else {
            return .blue
        }
        
        if stock == 0 {
            return Token.error300.swiftUIColor
        } else if stock < rop {
            return Token.warning400.swiftUIColor
        } else {
            return Token.success300.swiftUIColor
        }
    }
    
    // Logic ratio bar berdasarkan minStock
    private var progressRatio: CGFloat {
        guard let stock = product.stockAmount else { return 0 }
        guard let rop = product.rop else { return 0 }
        ...
        return CGFloat(stock) / CGFloat(rop)
    }
    ...
    private var cardContent: some View {
        HStack(alignment: .center, spacing: 12) {
            ...
            if let stock = product.stockAmount {
